...
- Sample: 1 bedroom apartments Manhattan
- Sample size: 20
...
**Interpretation**: We are 95% confident that the true mean rent price falls between 2162 and 2875.
...
- Population: Mice that died while not on treatment
...
**Interpretation**: We are 95% confident that the true proportion of mice that died while not on the treatment is between .69 and .797
...
boot_prop2 <- mice %>% 
  filter(treat == 1) %>% 
  mutate(died = if_else(died == 1, "no", "yes")) %>% 
  specify(response = died, success = "yes") %>% 
  generate(reps = 10000, type = "bootstrap") %>% 
  calculate(stat = "prop")
...
```{r practice_2_ci}
ci_mice_died <- get_ci(boot_prop2, level = .90)
ci_mice_died
```
...
We are 90% confident that the true proportion of mice that lived while on the treatment is between
.552 and .651.
